...
const data = [
  { icon: IconRun, label: 'Sales' },
  { icon: IconSwimming, label: 'New Orders' },
  { icon: IconBike, label: 'New Subscriptions' },
];

export function CalenderStates() {
  const { classes } = useStyles();
  const [date, setDate] = useState(new Date());

  const stats = data.map((stat) => (
    <Paper className={classes.stat} radius="md" shadow="md" p="md" key={stat.label}>
      <VStack>
      <stat.icon size={56} className={classes.icon} stroke={1.5}  />
